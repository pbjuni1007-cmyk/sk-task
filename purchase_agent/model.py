...
def model_settings():
    ...
    return {'api_key':values.get('OPENAI_API_KEY') or os.environ.get('OPENAI_API_KEY'),
...
def make_model():
    ...
    if not settings['api_key']:raise ValueError('MODEL_KEY_MISSING')
    # No tracing or implicit SDK retries: call attempts are counted by the agent.
    os.environ['LANGSMITH_TRACING']='false'
    os.environ['LANGCHAIN_TRACING_V2']='false'
    return ChatOpenAI(**settings,base_url='https://api.openai.com/v1',temperature=0,
...
def make_secondary_model():
    ...
    if not key or key==model_settings()['api_key']:
        return None
    return ChatOpenAI(api_key=key,model=model_settings()['model'],base_url='https://api.openai.com/v1',temperature=0,timeout=20,max_retries=0)
