...
from .schemas import CoupangSearchRequest
...
class MockCoupangClient:
    ...
    def search(self, request: CoupangSearchRequest, *, refresh=False, deadline=None):
        ...
        for attempt in range(2):
            ...
            self.rate_window.acquire(self.key)
            self.attempts += 1
            error = self.errors.popleft() if self.errors else None
            if error:
                if attempt == 0 and error.code in (429, 500, "timeout") and not error.retry_after:
                    continue
                raise error
            if request.keyword not in self._fixtures:
                raise LookupError("FIXTURE_NOT_CONFIGURED")
            result = self._fixtures[request.keyword].model_copy(deep=True)
            if request.srpLinkOnly:
                result.data.productData = None
            elif result.data.productData is None:
                raise ValueError("PRODUCT_LIST_MISSING")
            else:
                result.data.productData = result.data.productData[: request.limit]
