...
from purchase_agent.schemas import DecisionInput, ListQuery, RequestInput, RequestPatch, RequestRef
...
STATUS = dict(
    ...
    ready="제출 가능",
    submitted="구매팀 검토중",
    additional_approval="추가 결재 대기",
    approved="승인 완료",
    rejected="반려",
    needs_revision="보완 요청",
)
FOLDERS = dict(
    mine="내 요청함", pending="결재대기함", revision="보완요청함", completed="완료 문서함"
)
...
def inputs_form(inputs, key, submit_label="구매 조건 저장"):
    with st.form(key):
        ...
        budget = b.number_input(
            "예산 · 배송비 포함 (원)", min_value=1, value=inputs.budget_krw, step=10000
        )
...
def create_screen(service, context, state):
    ...
    st.caption(
        "저장하면 등록된 모니터 후보를 바로 검색합니다. 구매 목적은 제출 전까지 입력하면 됩니다."
    )
    values = inputs_form(
        RequestInput(quantity=1, budget_krw=300000), "create", "저장하고 상품 찾기"
    )
    if values:
        ...
        if request:
            ...
            if found.ok and found.data:
                state["notice"] = (
                    "조건을 저장했습니다. 아래 후보를 비교하고 구매할 상품을 선택하세요."
                )
            else:
                state["search_notice"] = (
                    "조건은 저장했지만 검색에 실패했습니다. 아래 후보 검색에서 다시 시도해 주세요."
                    if not found.ok
                    else "조건은 저장했지만 후보가 없습니다. 검색 조건을 바꿔 주세요."
                )
...
def list_screen(service, context, state):
    ...
    for detail in page.items:
        ...
        with st.container(border=True):
            ...
            row[1].caption(
                r.submitted_at.isoformat(timespec="minutes") if r.submitted_at else "제출일 —"
            )
...
def candidates_screen(service, context, state, detail):
    ...
    if "update" in detail.allowed_actions:
        ...
        selected = st.selectbox(
            "구매 상품 선택",
            ids,
            index=ids.index(r.selected_evidence_id) if r.selected_evidence_id in ids else None,
            format_func=lambda k: (
                f"{evidence[k].product.productName} · {evidence[k].supplement.option_label or '옵션 확인 필요'}"
            ),
            key=f"candidate_{r.version}",
        )
        if st.button("선택 상품 적용", disabled=selected is None):
            updated = result_data(
                service.update_request(
                    context,
                    r.request_id,
                    RequestPatch(expected_version=r.version, selected_evidence_id=selected),
                )
            )
            if updated:
                state["confirmation"] = None
...
def action_panel(service, context, state, detail):
    ...
    if r.owner_id == context.actor_id and "update" in detail.allowed_actions:
        ...
        if confirmation and (
            confirmation.version != r.version
            or not detail.documents
            or confirmation.bundle_id != detail.documents.bundle_id
            or not can_submit
        ):
            state["confirmation"] = None
            confirmation = None
        if confirmation:
            with st.container(border=True):
                ...
                selected = next(
                    e for e in detail.candidates if e.evidence_id == r.selected_evidence_id
                )
                st.text(
                    f"{r.request_id} · 버전 {r.version}\n{selected.product.productName} · {r.inputs.quantity}대\n배송비 포함 {money(detail.review.review_total_krw)}"
                )
                st.text(
                    "결재 경로: " + " → ".join(ROLES[a] for a in detail.review.required_approvers)
                )
                st.caption(
                    f"검토 문서: 버전 {detail.documents.version}의 구매요청서 · 상품 비교표 · 규정 검토"
                )
                a, b = st.columns(2)
                ...
                if b.button("제출", type="primary", key="confirm_submit"):
                    ...
                    if submitted:
                        state["confirmation"] = None
                        state["notice"] = (
                            f"제출 완료 · 최초 제출 시각 {submitted.submitted_at.isoformat(timespec='seconds')}"
                        )
                        st.rerun()
    decisions = [
        d for d in ("approve", "reject", "request_revision") if d in detail.allowed_actions
    ]
    if decisions:
        st.subheader("담당자 처리")
