...
def panel(service,context,state):
    ...
    for role,text in state.get('chat_history',[])[-10:]:
        with st.chat_message(role):st.text(text)
    ...
    if session and pending:
        ...
        for name,body in pending.documents.files.items():
            with st.expander(name):st.text(body)
        st.text(f'배송비 포함 총액: {pending.review.review_total_krw:,}원')
        a,b=st.columns(2)
        decision=None
        if a.button('AI 제출 취소',key='ai_cancel'):decision=False
        if b.button('확인 후 제출',key='ai_submit',type='primary'):decision=True
        if decision is not None:
            with st.spinner('확인 결과 처리 중…'):result=session.resume(decision)
            state['agent_result']=result
            if result.get('response'):state.setdefault('chat_history',[]).append(('assistant',result['response'].message))
            st.rerun()
        return
    with st.form('ai_chat'):
        text=st.text_area('AI 요청',placeholder='신입 3명용 모니터를 배송비 포함 90만 원 안에서 찾아줘')
    ...
    if sent and text.strip():
        ...
        if session is None:
            try:session=AgentSession(service,context);state['agent']=session
            except Exception:
                st.error('AI 연결 설정을 확인해 주세요.');return
        state.setdefault('chat_history',[]).append(('user',redact(text)))
        with st.spinner('구매 조건과 업무 상태를 확인 중…'):
            result=session.invoke(text,preference_consent=consent,request_id=state.get('request_id'))
        state['agent_result']=result
        if result.get('response'):state['chat_history'].append(('assistant',result['response'].message))
        if session.current_request:state['request_id']=session.current_request
        st.rerun()
    if session and session.current_request:
        if st.button('AI가 작업한 요청 열기'):
            state.update(view='detail',request_id=session.current_request,confirmation=None,decision=None)
            st.rerun()
