...
Status = Literal['draft', 'ready', 'submitted', 'additional_approval', 'approved', 'rejected', 'needs_revision']
...
class DocumentBundle(RequestRef):
    ...
    def all_documents_when_complete(self):
        if self.complete and (set(self.files) != {'purchase_request', 'comparison', 'review'} or not all(self.files.values())):
...
class PurchaseAssistantResponse(Model):
    status: Literal['needs_input', 'candidates_ready', 'needs_revision', 'documents_ready', 'submitted', 'blocked', 'failed']
    message: str
    request_id: str | None
