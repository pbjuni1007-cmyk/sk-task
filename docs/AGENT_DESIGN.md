...
| 20회 성능 p95≤30초 | 20/20성공, p50 6.2695초 / p95 6.922초 |
...
성능은 마지막 검색 전용 제한과 화면 정렬 보강 전 문서작성 경로의 측정이다. 해당 수정 후 여러 턴 실제 모델 스모크와 오프라인 회귀를 수행했으며20회 성능을 다시 측정하지 않았다. 114개 테스트가 원래38개 기준의 독립 실서비스 검증 전부를 의미하지 않는다. 공식 API 본문 독립 대조는 미완료다.
...
- 모의 클라이언트의50회/분 제한은 테스트 조건이며 실제 쿠팡 Search API 한도라고 주장하지 않는다.
...
- 공식 API 도움말 본문의 독립 대조는 접근 제한으로 미완료다. 실제 쿠팡 API 호출 검증을 완료했다고 주장하지 않는다.
