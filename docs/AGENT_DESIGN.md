...
구현 기준: 2026-09-10. 팀명·팀원은 제출자가 기입한다.

## 1. 개요

### 1.1 Agent 정의

**SK-TASK — Task Automation for Supplier Knowledge**는 사내 모니터 구매요청을 지원하는 Agent다. 직원이 수량·예산·구매 목적을 입력하면 상품 후보를 조회하고, 배송비를 포함한 예산과 구매 규정을 검토하여 구매요청서·상품 비교표·규정 검토 보고서를 작성한다. 사용자가 문서를 확인해 제출한 뒤 구매 담당자가 결재한다.
...
| 기능 | 구현 |
|---|---|
| 자연어 구매 조건 입력 | 단일 LangChain Agent와7개 도구 |
| 상품 조회·비교 | 쿠팡 파트너스 검색 형태의 모의 요청/응답; 사용자 확인 실제 상품2개 |
...
| 20회 성능 p95≤30초 | 브라우저 실행20/20성공, p50 6.602초 / p95 7.197초 |
| 업무·오류·권한 회귀 | 오프라인 테스트114개 통과 |
| 제출 확인 전 실행 금지 | 승인/취소 및 중복 제출 테스트 통과 |
| 문서3종 일관성 | 공통 요청/버전/수량/단가/총액 대조 통과 |
| 실제 상품 링크 | 사용자 확인 상품·옵션·이미지·배송 정보2개 일치 |

2026-09-10 최종 업무 코드로 Chrome에서 정상 문서 생성20회를 다시 실행했다. 시간은 Agent 처리 시간이다. 설계38개 기준은 일반 화면14개, 오류·가격·모델 주입 화면20개, QA 콘솔4개로 대조했다. 일반 화면에서 실제 API 장애를 모두 재현했다는 뜻은 아니다. 공식 API 본문 대조는 완료했으며, 브라우저 다운로드 저장 완료는 보안 정책 제한으로 미확인이다. 상세 근거는 [브라우저 검증 보고서](BROWSER_QA_REPORT.md)에 기록했다.
...
1. 수량·예산을 수집하고 요청을 저장한다. 기존 입력은 불필요하게 다시 묻지 않는다.
...
38개 설계 케이스와 실행 근거는 docs/TEST_TRACEABILITY.md에 연결했다. 추가된 경계까지 포함한 pytest114개가 통과했다. 오프라인 테스트는 네트워크 접속을 차단하며 결정적 모델로 도구 실행을 재현한다.
...
독립 검토에서 검색 전용 요청의 변경 가능성과 선호 정렬의 화면 미반영을 발견해 수정했다. 수정본과 직접 회귀 범위는 APPROVE이며 전체114개 테스트와7개 독립 회귀 검증으로 확인했다.

## 참고 자료 및 제출 전 확인

- README.md: 설치·실행·현재 제한
- docs/DEMO_GUIDE.md: 정상 구매·예산 초과·추가 승인 시연
- docs/TEST_TRACEABILITY.md: 설계 케이스별 근거
- 검증 결과: docs/evidence/의 측정 보고서와 테스트 로그
- [공식 API 본문 대조](COUPANG_API_CONTRACT_CHECK.md)는 완료했다. 실제 쿠팡 API 호출 검증은 수행하지 않았다.
