...
from test_workflow import catalog, service, OWNER, ready
from purchase_agent.config import demo_context
import pytest


def answer(**changes):
    data={'status':'needs_input','message':'수량을 알려주세요','request_id':None,'version':None,'missing_fields':['quantity'],'candidate_ids':[],'review_id':None,'document_bundle_id':None,'submitted_at':None,'warnings':['mock']}
    data.update(changes)
    return AIMessage(content='',tool_calls=[{'name':'PurchaseAssistantResponse','args':data,'id':'out'}])
...
def test_agent_submission_interrupt(service,approve,expected):
    ...
    model=ScriptModel(responses=[AIMessage(content='',tool_calls=[{'name':'submit_purchase_request','args':{'request_id':r.request_id,'version':r.version,'bundle_id':docs.bundle_id},'id':'submit'}]),answer()])
...
def test_call_budgets_and_redaction():
    b=CallBudget();b.reset()
    for _ in range(6):b.model_call()
    with pytest.raises(BudgetExceeded):b.model_call()
    for _ in range(10):b.tool_call()
    with pytest.raises(BudgetExceeded):b.tool_call()
    b.repair(ValueError())
    with pytest.raises(BudgetExceeded):b.repair(ValueError())
...
def test_store_reads_persistent_actor_preferences(service):
    ...
    with pytest.raises(PermissionError):store.get(('preferences','employee_b'),'settings')
    with pytest.raises(PermissionError):store.put(('preferences',OWNER.actor_id),'settings',{'output_style':'detailed'})

def test_summary_calls_count_toward_model_budget(service):
    model=ScriptModel(responses=[AIMessage(content='이전 대화 요약: 수량3, 예산90만 원'),answer()])
    session=AgentSession(service,OWNER,model)
    session.budget.reset()
    messages=[{'role':'user' if i%2==0 else 'assistant','content':f'이전 메시지{i}'} for i in range(24)]
...
def test_search_failure_does_not_poison_next_turn(service):
    r,_,_=ready(service)
    service.catalog=[]
    model=ScriptModel(responses=[AIMessage(content='',tool_calls=[{'name':'search_coupang_products','args':{'request_id':r.request_id,'version':r.version,'keyword':'monitor'},'id':'missing'}]),answer()])
    session=AgentSession(service,OWNER,model)
    assert session.invoke('후보 검색해줘',request_id=r.request_id)['response'].status=='needs_input'
...
def test_no_submission_without_user_intent(service):
    ...
    model=ScriptModel(responses=[AIMessage(content='',tool_calls=[{'name':'submit_purchase_request','args':{'request_id':r.request_id,'version':r.version,'bundle_id':docs.bundle_id},'id':'forbidden'}]),answer()])
    session=AgentSession(service,OWNER,model)
...
def test_review_prose_cannot_invent_price(service):
    ...
    model=ScriptModel(responses=[answer(status='needs_revision',message='단가는 100원입니다.',request_id=r.request_id,version=r.version,review_id=review.review_id,document_bundle_id=docs.bundle_id,missing_fields=[])])
    session=AgentSession(service,OWNER,model)
...
def test_unverified_prose_never_reaches_chat(service,status,has_request):
    ...
    model=ScriptModel(responses=[answer(status=status,message=forged,warnings=[forged],**args)])
    session=AgentSession(service,OWNER,model)
