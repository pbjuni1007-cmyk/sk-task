...
def test_nullable_purpose_and_patch_semantics():
    ...
    assert RequestPatch(expected_version=1).model_dump(exclude_unset=True) == {
        "expected_version": 1
    }
    assert (
        RequestPatch(expected_version=1, purpose=None).model_dump(exclude_unset=True)["purpose"]
        is None
    )
    with pytest.raises(ValidationError):
...
def test_response_modes_and_missing_price():
    envelope = {
        "rCode": "0",
        "rMessage": "",
        "data": {"landingUrl": "https://www.coupang.com/np/search?q=monitor"},
    }
    ...
    with pytest.raises(ValidationError):
        CoupangSearchResponse(**envelope)
    envelope["data"]["productData"] = []
...
def test_decision_reason_required(decision):
    with pytest.raises(ValidationError):
        DecisionInput(request_id="r", version=1, decision=decision, reason=" ")
...
def test_mock_search_limit_and_link_only_do_not_mutate_snapshot():
    ...
    envelope = CoupangSearchResponse(
        rCode="0",
        rMessage="",
        data={
            "landingUrl": "https://www.coupang.com/np/search?q=monitor",
            "productData": [product, dict(product, productId=2, rank=2)],
        },
    )
    client = MockCoupangClient({"monitor": envelope})
    assert (
        len(client.search(CoupangSearchRequest(keyword="monitor", limit=1)).data.productData) == 1
    )
    assert (
        client.search(CoupangSearchRequest(keyword="monitor", srpLinkOnly=True)).data.productData
        is None
    )
    assert len(client.search(CoupangSearchRequest(keyword="monitor")).data.productData) == 2
    with pytest.raises(LookupError):
        client.search(CoupangSearchRequest(keyword="unknown"))
