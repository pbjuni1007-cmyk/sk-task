...
from test_workflow import BUYER, OWNER, ready, submit
...
def test_exact_additional_approval_boundary(service, total, status):
    ...
    ref, _, docs = ready(service, 1, 2000000)
    submit(service, ref, docs)
    assert (
        service.decide(
            BUYER, DecisionInput(request_id=ref.request_id, version=ref.version, decision="approve")
        ).data.status
        == status
    )
...
def test_approved_version_cannot_authorize_revision(service):
    ...
    new = service.update_request(
        OWNER, ref.request_id, RequestPatch(expected_version=ref.version, quantity=4)
    ).data
...
def test_buyer_cannot_approve_own_request(service):
    ref = service.create_request(
        BUYER, RequestInput(quantity=1, budget_krw=500000, purpose="업무"), "self"
    ).data
    service.search_products(BUYER, ref, "monitor")
    ref = service.update_request(
        BUYER, ref.request_id, RequestPatch(expected_version=ref.version, selected_evidence_id="e1")
    ).data
    service.review_request(BUYER, ref)
    docs = service.generate_documents(BUYER, ref).data
    c = service.prepare_submission(BUYER, ref, docs.bundle_id).data
    assert service.submit_request(BUYER, c).ok
    assert not service.decide(
        BUYER, DecisionInput(request_id=ref.request_id, version=ref.version, decision="approve")
    ).ok
...
@pytest.mark.parametrize(
    "quantity,fee,rule,budget,expected",
    [(2, 3000, "per_order", 500000, 501000), (4, 1000, "per_item", 1100000, 1000000)],
)
def test_shipping_changes_budget_and_approval(service, quantity, fee, rule, budget, expected):
    ...
    if expected > budget:
        assert (
            not review.can_submit and not service.prepare_submission(OWNER, ref, docs.bundle_id).ok
        )
    else:
        submit(service, ref, docs)
        assert (
            service.decide(
                BUYER,
                DecisionInput(request_id=ref.request_id, version=ref.version, decision="approve"),
            ).data.status
            == "additional_approval"
        )
...
def test_second_turn_preserves_selection(service):
    ...
    assert (
        latest.request.selected_evidence_id == "e1" and latest.request.inputs.purpose == "개발 업무"
    )
...
def test_user_catalog_matches_source():
    ...
    for e, s in zip(evidence, source):
        ...
        assert (
            str(e.product.productUrl) == s["productUrl"]
            and str(e.product.productImage) == s["productImage"]
        )
        assert (
            e.supplement.item_id == s["itemId"] and e.supplement.vendor_item_id == s["vendorItemId"]
        )
    ...
    class ScriptedClient(MockCoupangClient):
        def __init__(self, *args, **kwargs):
            super().__init__(
                *args,
                **kwargs,
                errors=[SearchError(code)] if code == 500 else [SearchError(code)] * 2,
            )
...
def test_descriptive_monitor_query_and_selection_keys(service):
    ...
    result = search.invoke(
        {"request_id": ref.request_id, "version": ref.version, "keyword": "업무용 모니터"}
    )
...
def test_summary_preserves_db_conditions_and_actor_isolation(service):
    ...
    detail = service.get_latest_request(OWNER, ref.request_id).data
    assert (
        detail.request.inputs.quantity,
        detail.request.inputs.budget_krw,
        detail.request.selected_evidence_id,
    ) == (3, 900000, "e1")
    other = AgentSession(
        service,
        demo_context("employee_b", "other"),
        ScriptModel(
            responses=[
                call("get_request_status", request_id=ref.request_id),
...
def test_document_success_uses_server_identifiers(service):
    ...
    model = ScriptModel(
        responses=[
            call("generate_documents", request_id=ref.request_id, version=ref.version),
            answer(
                status="documents_ready",
                request_id=ref.request_id,
                version=ref.version,
                document_bundle_id="invented",
            ),
        ]
    )
    session = AgentSession(service, OWNER, model)
    ...
    latest = service.get_latest_request(OWNER, ref.request_id).data
    assert (
        result.status == "documents_ready"
        and result.document_bundle_id == latest.documents.bundle_id
    )
...
def test_third_document_failure_persists_no_partial_bundle(service, monkeypatch):
    ...
    def fail_on_review(value):
        if value == "목적·선택 상품·기본 입력 확인":
            raise ValueError("THIRD_DOCUMENT_RENDER_FAILED")
        return original(value)

    ref = service.create_request(
        OWNER,
        RequestInput(quantity=3, budget_krw=900000, purpose="개발 업무"),
        "third-document-failure",
    ).data
    service.search_products(OWNER, ref, "monitor")
    ref = service.update_request(
        OWNER, ref.request_id, RequestPatch(expected_version=ref.version, selected_evidence_id="e1")
    ).data
...
def test_missing_price_wire_response_stops_agent(service, monkeypatch):
    ...
    ref = service.create_request(
        OWNER, RequestInput(quantity=1, budget_krw=500000), "bad-wire"
    ).data
    agent = AgentSession(
        service,
        OWNER,
    ...
    assert (
        agent.budget.tools == 1
        and not service.get_latest_request(OWNER, ref.request_id).data.documents
    )
...
def test_agent_purpose_missing_blocks_submission(service):
    ...
    agent = AgentSession(
        ...
        OWNER,
        ScriptModel(
            responses=[answer(status="needs_input", request_id=ref.request_id, version=ref.version)]
        ),
    )
...
def test_consented_preferences_apply_in_new_session(service):
    ...
    assert service.save_preferences(
        OWNER, {"output_style": "detailed", "comparison_priority": "price"}, consent=True
    ).ok
    ref, review, docs = ready(service)
    agent = AgentSession(
        service,
    ...
    result = search.invoke(
        {"request_id": ref.request_id, "version": ref.version, "keyword": "monitor"}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("upsert_purchase_request", {"selected_evidence_id": "e2"}),
        ("upsert_purchase_request", {"clear_selection": True}),
    ],
)
def test_search_only_cannot_change_selection_or_documents(service, name, args):
    ...
    kwargs = {
        "request_id": ref.request_id,
        ("expected_version" if name == "upsert_purchase_request" else "version"): ref.version,
        **args,
    }
    agent = AgentSession(
        service,
        OWNER,
        ScriptModel(
            responses=[call(name, **kwargs), answer(request_id=ref.request_id, version=ref.version)]
        ),
    )
