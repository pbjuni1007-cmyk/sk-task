...
from purchase_agent.coupang_client import MockCoupangClient, RateWindow, SearchError
...
def client(**kwargs):
    return MockCoupangClient(
        {
            "x": CoupangSearchResponse(
                rCode="0",
                rMessage="",
                data={"landingUrl": "https://www.coupang.com/np/search", "productData": []},
            )
        },
        **kwargs,
    )
...
def test_recover_once_and_cache():
    ...
    c.search(Q)
    assert c.attempts == 2
    c.search(Q, refresh=True)
    assert c.attempts == 3


def test_shared_rate_window_and_reset():
    now = [0.0]
    window = RateWindow(clock=lambda: now[0])
    a = client(rate_window=window)
    b = client(rate_window=window)
    for _ in range(50):
        a.search(Q, refresh=True)
    with pytest.raises(SearchError):
        b.search(Q)
    now[0] = 60
