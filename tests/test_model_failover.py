...
from purchase_agent.middleware import BudgetExceeded, BudgetModel, CallBudget
...
def error(code):
    return APIStatusError(
        ...
        response=httpx.Response(
            code, request=httpx.Request("POST", "https://api.openai.com/v1/chat/completions")
        ),
        body={},
    )
...
def test_unrelated_error_not_retried_with_another_key(code):
    ...
    with pytest.raises(APIStatusError):
        BudgetModel(inner=primary, secondary=backup, budget=budget).invoke("test")
