...
@pytest.fixture
def catalog():
    return [ProductEvidence(evidence_id=f'e{i}',product=CoupangProduct(productId=i,productName=f'Synthetic monitor {i}',productPrice=p,productUrl=f'https://www.coupang.com/vp/products/{i}',productImage='https://example.com/test.png',isRocket=False,isFreeShipping=True,keyword='monitor',rank=i),supplement=ProductSupplement(product_id=i,item_id=str(i),vendor_item_id=str(i),option_label='27 QHD',verified_specs=['27 QHD'],shipping_rule='free',shipping_source_url=f'https://www.coupang.com/vp/products/{i}',source_checked_at=datetime.now(timezone.utc),source_status='verified'),query='monitor',retrieved_at=datetime.now(timezone.utc),fixture_version='test') for i,p in [(1,280000),(2,250000)]]

@pytest.fixture
def service(tmp_path,catalog):return LocalPurchaseService(tmp_path/'test.db',catalog)
...
def ready(service,quantity=3,budget=900000,purpose='개발 업무'):
    r=service.create_request(OWNER,RequestInput(quantity=quantity,budget_krw=budget,purpose=purpose),'create').data
...
def test_complete_normal_and_duplicate(service):
    ...
    approved=service.decide(BUYER,DecisionInput(request_id=r.request_id,version=r.version,decision='approve'))
...
def test_purpose_optional_until_submit(service):
    ...
    changed=service.update_request(OWNER,r.request_id,RequestPatch(expected_version=r.version,purpose='업무')).data
...
def test_high_amount(service):
    ...
    assert service.decide(BUYER,DecisionInput(request_id=r.request_id,version=r.version,decision='approve')).data.status=='additional_approval'
    assert service.decide(MANAGER,DecisionInput(request_id=r.request_id,version=r.version,decision='approve')).data.status=='approved'
...
def test_ownership_and_nonce_thread(service):
    ...
    assert not service.decide(OWNER,DecisionInput(request_id=r.request_id,version=r.version,decision='approve')).ok
...
def test_concurrent_submission(service):
    r,_,docs=ready(service)
    c=service.prepare_submission(OWNER,r,docs.bundle_id).data
    with ThreadPoolExecutor(2) as pool:results=list(pool.map(lambda _:service.submit_request(OWNER,c),range(2)))
...
def test_change_preserves_old_docs(service):
    r,_,docs=ready(service)
    submit(service,r,docs)
    new=service.update_request(OWNER,r.request_id,RequestPatch(expected_version=r.version,quantity=4)).data
    ...
    assert not service.decide(BUYER,DecisionInput(request_id=r.request_id,version=r.version,decision='approve')).ok
...
def test_preferences(service):
    ...
    assert service.get_preferences(demo_context('employee_a','next')).data['output_style']=='concise'
    assert service.get_preferences(demo_context('employee_b','s')).data=={}

@pytest.mark.parametrize('price,quantity,fee,rule,total',[(249000,2,3000,'per_order',501000),(249000,4,1000,'per_item',1000000)])
...
def test_comparison_threshold(service,total,expected):
    service.catalog=service.catalog[:1];service.catalog[0].product.productPrice=total
    _,review,_=ready(service,1,600000)
    assert review.can_submit==expected

def test_render_failure_rollback(service,monkeypatch):
    r=service.create_request(OWNER,RequestInput(quantity=3,budget_krw=900000,purpose='업무'),'a').data
    service.search_products(OWNER,r,'monitor')
    r=service.update_request(OWNER,r.request_id,RequestPatch(expected_version=r.version,selected_evidence_id='e1')).data
    service.review_request(OWNER,r)
    def fail(_):raise ValueError('RENDER_FAILED')
...
def test_document_tamper_blocks_submit(service):
    r,_,docs=ready(service);c=service.prepare_submission(OWNER,r,docs.bundle_id).data
...
def test_revision_must_not_overwrite_submitted_documents(service):
    ...
    assert service.decide(BUYER,DecisionInput(request_id=r.request_id,version=r.version,decision='request_revision',reason='목적 구체화')).ok
    assert service.generate_documents(OWNER,r).data==docs
    new=service.update_request(OWNER,r.request_id,RequestPatch(expected_version=r.version,purpose='개발팀 온보딩 업무')).data
    assert new.version==r.version+1 and new.status=='draft'

def test_negative_spec_is_not_positive_match(service):
    for e in service.catalog:e.supplement.verified_specs=['USB-C 미지원']
    r=service.create_request(OWNER,RequestInput(quantity=2,budget_krw=600000,purpose='업무',requirements=['USB-C']),'spec').data
    service.search_products(OWNER,r,'monitor')
    r=service.update_request(OWNER,r.request_id,RequestPatch(expected_version=r.version,selected_evidence_id='e1')).data
...
def test_concurrent_approval_changes_state_once(service):
    ...
    with ThreadPoolExecutor(2) as pool:results=list(pool.map(lambda _:service.decide(BUYER,decision),range(2)))
    assert sum(x.ok for x in results)==1
    assert len(service.get_request(OWNER,r).data.history)==2

def test_shipping_only_change_creates_new_version(service):
    r,_,docs=ready(service)
    service.catalog[0].supplement.shipping_rule='per_order';service.catalog[0].supplement.fee_krw=3000
    assert service.search_products(OWNER,r,'monitor',refresh=True).ok
...
def test_submit_revision_race_is_serialized(service):
    ...
    with ThreadPoolExecutor(2) as pool:
        ...
        update_future=pool.submit(service.update_request,OWNER,r.request_id,RequestPatch(expected_version=r.version,quantity=4))
...
def test_same_product_different_option_respects_wire_limit(service):
    ...
    other.evidence_id='other';other.product.productPrice=400000
    other.product.productUrl=HttpUrl('https://www.coupang.com/vp/products/1?itemId=other')
    other.supplement.item_id='other';other.supplement.option_label='32 QHD'
    ...
    assert [(e.evidence_id,e.product.productPrice) for e in both]==[('e1',280000),('other',400000)]
