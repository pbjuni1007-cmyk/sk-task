...
with tempfile.TemporaryDirectory() as directory:
    ...
    result = session.invoke(
        "개발팀 신입 3명 업무용 모니터를 배송비 포함 90만원 이내로 구매하려고 해. 수량과 예산, 목적을 저장하고 후보를 찾아줘. 상품 자료가 없으면 없다고 안내해."
    )
    ...
    print(
        {
            ...
            "saved_requests": saved.total,
            "metrics": result["metrics"],
            "tool_results": session.budget.tool_results,
        }
    )
    if output is None or output.status == "failed" or saved.total != 1:
        sys.exit(1)
